...
<# ------ 草稿
    ...
    powershell 原生不支持直接导出xlsx：Install-Module -Name ImportExcel 。
    Export-Excel -Path "C:\events.xlsx" -AutoSize 不用关心编码问题
    没有 ImportExcel 模块就先安装再说
...
<# ------ 过时 Get-WmiObject，转 Get-CimInstance 
...
<# ------- 调整
    ...
    # Write-Host "--- 最近一周系统唤醒频次 ---"  -ForegroundColor Yellow
    ...
    #     $result | Out-GridView -Title "一周的唤醒频次近况"
    #     $sum = ($result | Measure-Object).Count
    #     Write-Host "最近7天唤醒总计:"$sum, "`n最近7天平均每天唤醒系统次数:"$([math]::Round($sum / 7, 2)) -ForegroundColor Green
    ...
    #     $maxDate = ($dateCounts.GetEnumerator() | Sort-Object -Property Value -Descending | Select-Object -First 1).Name
    #     $maxCount = $dateCounts[$maxDate]
    #     Write-Host "唤醒系统最多频次的日期: $maxDate, 以及其唤醒次数: $maxCount" -ForegroundColor Cyan

    # }
    # else {
    #     Write-Host "没有找到最近7天的锁屏唤醒系统数据。"-ForegroundColor DarkRed
    # }
...
<#----- 保留基础部分
    ...
    # } `
    # | Where-Object {$_.LevelDisplayName -eq "警告" -or $_.LevelDisplayName -eq "错误"} `
    # | Out-GridView -Title "近期及当前异常警告和错误事件分析表"
