...
function check_fw {
    ...
    Write-Host "--- 检查是否存在评估版自动重启设置注册表（企业版） ---`n" -ForegroundColor Yellow
    if (Test-Path -Path "HKLM:\SYSTEM\CurrentControlSet\Services\WLMS") {
        ...
        Write-Host "windows 系统每隔一段时间会自动关机解决办法：" -ForegroundColor Yellow -NoNewline
        Write-Host "http://www.yvsy.com/501.html" -ForegroundColor Blue
        $result | Select-Object ImagePath, Start | Format-List
    } else {
        Write-Host "没有评估版的相关注册表`n" -ForegroundColor Green
    }
    ...
    Write-Host "主机正常情况下存有该四项：FxsTmp、Recovery、SleepStudy、WDI（虚拟机无FxsTmp）`n"
